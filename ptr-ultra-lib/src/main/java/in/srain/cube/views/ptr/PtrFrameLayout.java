 ...
 * you want. support: pull to refresh / release to refresh / auto refresh / keep header view while
 * refreshing / hide header view while refreshing It defines {@link
 * in.srain.cube.views.ptr.PtrUIHandler}, which allows you customize the UI easily.
 */
    ...
    switch (action) {
      ...
      case MotionEvent.ACTION_MOVE:
        ...
        if (DEBUG) {
          ...
          PtrCLog.v(
              LOG_TAG,
              "ACTION_MOVE: offsetY:%s, currentPos: %s, moveUp: %s, canMoveUp: %s, moveDown: %s: canMoveDown: %s,mPreventForHorizontal: %s",
              offsetY,
              mPtrIndicator.getCurrentPosY(),
              moveUp,
              canMoveUp,
      ...
      if (mKeepHeaderWhenRefresh) {
        ...
        if (mPtrIndicator.isOverOffsetToKeepHeaderWhileLoading() && !stayForLoading) {
          mScrollChecker.tryToScrollTo(
              mPtrIndicator.getOffsetToKeepHeaderWhileLoading(), mDurationToClose);
        } else {
          // do nothing
        }
      } else {
