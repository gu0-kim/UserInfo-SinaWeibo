...
/**
 * Fancy progress indicator for Material theme. It's taken from {@link android.support.v4.widget}.
...
public class MaterialProgressDrawable extends Drawable implements Animatable {
  ...
  public void updateSizes(int size) {
    if (size == LARGE) {
      ...
      setSizeParameters(
          CIRCLE_DIAMETER, CIRCLE_DIAMETER, CENTER_RADIUS, STROKE_WIDTH, ARROW_WIDTH, ARROW_HEIGHT);
    }
  }
  ...
  private void setupAnimators() {
    final Ring ring = mRing;
    final Animation finishRingAnimation =
        new Animation() {
        ...
        new Animation() {
          ...
          public void applyTransformation(float interpolatedTime, Transformation t) {
            ...
            final float minProgressArc =
                (float)
                    Math.toRadians(ring.getStrokeWidth() / (2 * Math.PI * ring.getCenterRadius()));
            ...
            ring.setRotation(rotation);
            float groupRotation =
                ((720.0f / NUM_POINTS) * interpolatedTime)
