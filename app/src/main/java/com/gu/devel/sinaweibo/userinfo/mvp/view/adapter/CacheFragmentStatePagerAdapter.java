 ...
 *
 * <p>FragmentStatePagerAdapter is also originally caches pages, but its keys are not public nor
 * documented, so depending on how it create cache key is dangerous.
 *
 * <p>This adapter caches pages by itself and provide getter method to the cache.
 */
public abstract class CacheFragmentStatePagerAdapter<T extends Fragment>
  ...
  public void restoreState(Parcelable state, ClassLoader loader) {
    ...
    if (0 < pages) {
      for (int i = 0; i < pages; i++) {
        ...
        T f = (T) mFm.getFragment(bundle, createCacheKey(position));
  ...
  /**
   * Create a new Fragment instance. This is called inside {@link #getItem(int)}.
   *
   * @param position Position of the item in the adapter.
   * @return Fragment instance.
   */
  protected abstract T createItem(int position);
